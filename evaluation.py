...
from train import (forward_euler,trainer)
...
def plot_performance():
    ...
    p2s = np.array(torch.load('metrics/phase_1_eval_p2s_acc'))
    s2p = np.array(torch.load('metrics/phase_1_eval_s2p_acc')) 
    ...
    o2s = np.array(torch.load('metrics/phase_1_eval_o2s_acc'))
    o2p = np.array(torch.load('metrics/phase_1_eval_o2p_acc')) 
       ...
       with torch.no_grad():
           for start_step in range(2,12):
               ...
               for idx in tqdm.tqdm(range(len(orthography)),position=0):
                   ...
                   predicted_phonology,_ = trainer(model,{'orthography':orthography})
...
def plot_strain(results,filepath):
    ...
    for ckpt in tqdm.tqdm(glob.glob('ckpts/phase_2*'),position=0):
       ...
       os.makedirs(f'figures/glushko/{ckpt}',exist_ok=True)
          with torch.no_grad()::
              ...
              results = np.array(results)
              plot_strain(results,f'figures/glushko/{ckpt}')
    
def plot_development(results,filename):
    
    plt.figure(figsize=(15,8),dpi=400)
    x = np.linspace(0,4,results[-1].shape[0])
    ...
    for ckpt in tqdm.tqdm(glob.glob('ckpts/phase_2*'),position=0):
       ...
       for data in dataset:
           with torch.no_grad():
               ...
               outputs = trainer(model,{'orthography':data['orthography'][None]},delta_t=1/12,return_outputs=True)
...
if __name__=='__main__':
   ...
   global dataset
   phoneme_embeddings = dataset.dataset.phonology_tokenizer.embedding_table.to_numpy()
   ...
   ckpts = tqdm.tqdm(glob.glob('ckpts/phase_2*')    
